...
| `size` | string | none | Modal size: `'small'`, `'medium'`, `'large'`, `'fullscreen'` |
...
The Modal overlay centers the modal wrapper using flex (`justifyContent: 'center'`, `alignItems: 'center'`). In normal-sized modals this is automatic. In `fullscreen` mode, the wrapper fills the entire screen, so **drop components must center their own content**.

For fullscreen drops, add these styles to center horizontally:
```javascript
container: {
  flex: 1,
  alignItems: 'center',     // centers children horizontally
},
// For rows of chips/buttons:
row: {
  flexDirection: 'row',
...
grid: {
  flexDirection: 'row',
  flexWrap: 'wrap',
  justifyContent: 'center',  // centers wrapped rows
},
```

For normal (non-fullscreen) drops, the Modal handles centering — drop components don't need special centering styles.

### Fullscreen Mode

Set `size: 'fullscreen'` on a drop or the flow definition. This:
- Removes the title/nav bar
- Sets `width: 100%`, `height: 100%`, no border radius
- Shows only a small close button (top-right, 70% opacity)
- Falls back from flow-level: `drop.size || flowDefinition.size`

Fullscreen is useful for games and editors that need maximum screen space. The Create Board dialog in Pixel Pals overrides with `size: 'medium'` at `depth: 1` to appear as a normal modal on top of the fullscreen landing.
