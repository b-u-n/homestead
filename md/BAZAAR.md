...
### Account.assetCustomizations (added to Account model)

```
assetCustomizations: [{
  platformAssetId: String (required),
  shopItemId: ObjectId ref ShopItem (required),
  revisionIndex: Number (required),
  contentUrl: String,
  itemTitle: String,
  appliedAt: Date
}]
...
One entry per `platformAssetId` (upsert on set). See `md/CUSTOMIZATION.md` for full documentation.
...
| Event | Auth | Description |
|-------|------|-------------|
| `bazaar:purchases:mine` | Session | List user's purchased items, enriched with `platformAssetId` |
| `bazaar:purchases:revisions` | Session | Get approved revisions for an owned item |
| `bazaar:customization:set` | Session | Apply a revision to replace a platform asset |
| `bazaar:customization:list` | Session | List user's active customizations |
| `bazaar:customization:clear` | Session | Remove a customization (revert to default) |
...
```
customizationTable:assetPicker (startAt — browse platform assets)
  └─ customizationTable:itemPicker [depth 1] (pick from purchased items)
      └─ customizationTable:revisionPicker [depth 1] (pick revision to apply)
```
...
| Customization Table frontend flow | `/frontend/flows/customizationTableFlow.js` |
| Customization Table wrapper | `/frontend/components/CustomizationTable.js` |
| CustomizationStore | `/frontend/stores/CustomizationStore.js` |
| Customization documentation | `/md/CUSTOMIZATION.md` |
| Art style guide | `/md/ART_STYLE.md` |
